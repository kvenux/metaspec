...
if (-not $MetaSpecRoot) {
  $MetaSpecRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..")).Path
}
if (-not $SourceRepo) {
  $SourceRepo = (Resolve-Path -LiteralPath (Join-Path $MetaSpecRoot "..\repos\spring-petclinic")).Path
}
if (-not $RunRoot) {
  $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
  $RunRoot = Join-Path $MetaSpecRoot "runs\clarification-$stamp"
}

$CaseName = "case-owner-phone-lookup"
$ChangeName = "REQ20260507-owner-phone-lookup"
$MetaSpecBin = Join-Path $MetaSpecRoot "bin\metaspec.js"
...
function Copy-Project([string]$Destination) {
  ...
  & git -C $Destination config user.email "metaspec-experiment@example.local" | Out-Null
  & git -C $Destination config user.name "MetaSpec Experiment" | Out-Null
...
function Invoke-MetaSpec([string[]]$CliArgs) {
  $result = & node $MetaSpecBin @CliArgs 2>&1
  if ($LASTEXITCODE -ne 0) {
    throw "metaspec failed: $($result -join "`n")"
  }
  return $result
}

function Add-OptimizedMetaSpecRules([string]$Repo) {
  $block = @'
...
Always obey the base MetaSpec state machine: first call `metaspec go --json`, write only to `stage.allowedWritePath`, never create a root-level `proposal.md`, and never call `metaspec accept --json` before the user explicitly confirms the written artifact.
...
'@
  ...
  foreach ($relative in @(
      ".agents\skills\metaspec\SKILL.md",
      ".agents\skills\metaspec-proposal\SKILL.md"
    )) {
...
function Setup-Variant([string]$Variant, [string]$VariantDir) {
  ...
  if ($Variant -in @("with-baseline", "with-optimized")) {
    Invoke-MetaSpec -CliArgs @("init", $repo, "--integration", "codex", "--json") | Out-Null
    Invoke-MetaSpec -CliArgs @("--path", $repo, "start", $ChangeName, "--json") | Out-Null
    if (-not (Test-Path -LiteralPath (Join-Path $repo ".agents\skills\metaspec\SKILL.md"))) {
      throw "MetaSpec Codex skill was not installed in $repo"
    }
    if ($Variant -eq "with-optimized") {
      Add-OptimizedMetaSpecRules $repo
    }
  }
...
function Get-InitialPrompt([string]$Variant) {
  ...
  if ($Variant -eq "without") {
    return @(
      ...
      "",
      "Do not use MetaSpec. Do not use Superpowers. Do not implement code.",
  ...
  if ($Variant -eq "with-superpowers") {
    return @(
      ...
      "",
      "This is a controlled requirement-clarification experiment.",
      "Do not use MetaSpec. Do not implement code. User instructions override the default Superpowers spec location for this experiment.",
      "",
  ...
  return @(
    "Use the MetaSpec repository skill to run the active MetaSpec proposal stage.",
    "",
    "This is a controlled requirement-clarification experiment.",
    "Do not implement code.",
    "Do not write proposal.md until you ask for generation approval and I reply exactly generate.",
    "After writing proposal.md, do not call metaspec accept --json until I reply exactly confirm.",
...
function Get-StageStatus([string]$Repo) {
  try {
    $text = & node $MetaSpecBin "--path" $Repo "status" "--json" 2>$null
...
function Get-NextUserReply([string]$Variant, [string]$Repo, [string]$Message) {
  ...
  $proposalFile = Get-ChildItem -Path (Join-Path $Repo "metaspec\changes") -Recurse -Filter "proposal.md" -ErrorAction SilentlyContinue | Select-Object -First 1
...
function Collect-Artifacts([string]$Variant, [string]$Repo, [string]$VariantDir) {
  ...
  if ($Variant -in @("with-baseline", "with-optimized")) {
    $changeDir = Get-ChildItem -Path (Join-Path $Repo "metaspec\changes") -Directory -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -ne "archives" } |
      Select-Object -First 1
    if ($changeDir) {
      Copy-Item -LiteralPath $changeDir.FullName -Destination (Join-Path $artifactDir "metaspec-change") -Recurse -Force
    }
    $status = Get-StageStatus $Repo
    if ($status) {
      $status | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath (Join-Path $artifactDir "metaspec-status.json") -Encoding UTF8
    }
  }
  else {
