...
export function resolveGenerationStrategy(paths, options = {}, env = process.env, context = {}) {
  ...
  const requestedMode = normalizeProvider(options.mode || env.METASPEC_GENERATION_MODE || "auto");
  if (!["auto", "direct", "react"].includes(requestedMode)) {
    return {
      ...
      next: ["metaspec generate --mode auto"]
    };
  }
  ...
  if (runner === "opencode") {
    return {
      ...
      next: ["metaspec generate --runner auto", "metaspec generate --runner codex", "metaspec generate --runner claude"]
    };
  }
  if (!["auto", "codex", "claude"].includes(runner)) {
    return {
      ...
      next: ["metaspec generate --runner auto"]
    };
  }

  const provider = normalizeProvider(env.METASPEC_LLM_PROVIDER || env.LLM_PROVIDER || readConfigProvider(paths));
  if (provider === "fake") {
    ...
    return {
      ...
      runner: "auto",
      provider: "fake",
      model: options.model || env.METASPEC_LLM_MODEL || env.LLM_MODEL || "fake-metaspec-model"
    };
  }

  if (["openai", "anthropic"].includes(provider)) {
    const apiKey = env.METASPEC_LLM_API_KEY || env.LLM_API_KEY;
    if (!apiKey) {
      return {
        ok: false,
        code: "LLM_NOT_CONFIGURED",
        provider,
        message: `Missing ${provider} LLM configuration. Set METASPEC_LLM_API_KEY or use METASPEC_LLM_PROVIDER=fake.`,
        next: ["unset METASPEC_LLM_PROVIDER", "metaspec generate --runner auto"]
      };
    }
  }
...
function selectRunner(requestedRunner, env) {
  ...
  if (!executable) {
    return {
      ...
      message: `${requestedRunner} was not found. Install and authenticate the CLI, or use metaspec generate --runner auto.`,
      next: ["metaspec generate --runner auto", `Confirm ${requestedRunner} is installed and authenticated`]
    };
  }
...
`,
  ...
  if (task === "design") {
    const modules = plan.modules
      ...
      .join("\n");
    return {
      text: `# MetaSpec Implementation Design

<!-- generated by metaspec fake direct -->
...
MetaSpec CLI owns artifact writing and apply protection.
