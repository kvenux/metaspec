  ...
  if (["openai", "anthropic"].includes(provider)) {
    ...
    if (!apiKey) {
      return {
        ...
        provider,
        message: `缺少 ${provider} LLM 配置。请设置 METASPEC_LLM_API_KEY 或改用 METASPEC_LLM_PROVIDER=fake。`,
...
function selectRunner(requestedRunner, env) {
  ...
  if (!executable) {
    return {
      ...
      message: `未找到 ${requestedRunner}。请安装并登录对应 CLI，或改用 metaspec generate --runner auto。`,
      next: ["metaspec generate --runner auto", `确认 ${requestedRunner} 已安装并登录`]
    };
  }
