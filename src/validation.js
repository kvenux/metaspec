...
export function validateProject(options = {}, explicitChange) {
  ...
  if (!fs.existsSync(path.join(paths.runtime, "config.yaml"))) {
    findings.push(finding("error", "CS007", ".codespec-cli/config.yaml", "缺少 .codespec-cli/config.yaml。"));
  }
  const dataDir = path.join(paths.codespec, "data");
  if (fs.existsSync(dataDir)) {
    const jsonFiles = fs.readdirSync(dataDir).filter((name) => name.endsWith(".json"));
    for (const name of jsonFiles) findings.push(finding("error", "CS008", `codespec/data/${name}`, "禁止把 JSON 数据文件作为核心规格资产。"));
  }
...
function validateChange(root, change, findings) {
  const dir = path.join(root, "codespec/changes", change);
  if (!fs.existsSync(dir)) {
    findings.push(finding("error", "CS100", `codespec/changes/${change}`, "指定变更目录不存在。"));
    return;
  }
  ...
  for (const stage of STAGES) {
    ...
    if (exists && !previousConfirmed) {
      findings.push(finding("warn", "CS102", `codespec/changes/${change}/${stage.file}`, "后序阶段文件存在但前序阶段未完成。"));
    }
    if (exists && status !== "confirmed" && status !== "template") {
      findings.push(finding("warn", "CS103", `codespec/changes/${change}/${stage.file}`, "阶段文件未经过用户确认。"));
    }
...
function validateDeltaSpec(file, change, findings) {
  ...
  for (const title of ["ADDED Requirements", "MODIFIED Requirements", "REMOVED Requirements"]) {
    if (!content.includes(title)) {
      findings.push(finding("warn", "CS201", `codespec/changes/${change}/delta-spec.md`, `delta-spec.md 缺少 ${title} 标题。`));
    }
  }
}
...
function validateTasks(file, change, findings) {
  ...
  if (!/测试|验证|test|validation/i.test(content)) {
    findings.push(finding("warn", "CS301", `codespec/changes/${change}/tasks.md`, "tasks.md 未引用或包含验证任务。"));
  }
}
...
function validateValidation(file, change, findings) {
  ...
  if (!/允许进入实现|可进入实现|进入实现/.test(content)) {
    findings.push(finding("warn", "CS401", `codespec/changes/${change}/validation.md`, "validation.md 缺少进入实现结论。"));
  }
}
