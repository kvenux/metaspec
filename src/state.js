...
export function acceptStage(options = {}, explicitChange, explicitStage) {
  ...
  return {
    ...
    next: next
      ...
      : isZh(options) ? ["执行实现任务", "运行必要验证", "done finalization 更新 full spec/design 后执行 metaspec done"] : ["Implement the tasks", "Run required verification", "Run metaspec done after done finalization updates full spec/design"]
  };
}
...
export function validateFullDocumentUpdatesForDone(options = {}, explicitChange) {
  ...
  if (!baseline?.documents) {
    return {
      ...
      message: tr(options, "Missing implementation baseline snapshot. Confirm validation.md again or use metaspec archive --force only for manual recovery.", "缺少实现前 baseline 快照。请重新确认 validation.md，或仅在手工恢复时使用 metaspec archive --force。")
    };
  }

  const current = captureFullDocumentBaseline(paths.root);
  const required = ["metaspec/specs/spec.md", "metaspec/specs/design.md"];
  const notUpdated = [];
  ...
  if (notUpdated.length) {
    return {
      ...
      message: tr(options, "Done finalization evidence is incomplete. metaspec/specs/spec.md and metaspec/specs/design.md must both be updated after validation before archive/done.", "done finalization 证据不完整。archive/done 前，metaspec/specs/spec.md 和 metaspec/specs/design.md 都必须在 validation 后更新。"),
      notUpdated,
      notMerged: notUpdated,
      next: isZh(options)
        ? ["done finalization: 更新 metaspec/specs/spec.md", "done finalization: 更新 metaspec/specs/design.md", "确认测试结果后再执行 metaspec done"]
        : ["Done finalization: update metaspec/specs/spec.md", "Done finalization: update metaspec/specs/design.md", "Confirm verification, then run metaspec done"]
    };
  }
...
function captureFullDocumentBaseline(root) {
  const documents = {};
  for (const filePath of ["metaspec/specs/spec.md", "metaspec/specs/design.md"]) {
...
export function archiveChange(options = {}, explicitChange) {
  ...
  return {
    ...
    next: isZh(options) ? ["变更已归档", "继续下一个 metaspec change"] : ["Change archived", "Continue with the next MetaSpec change"]
  };
}
